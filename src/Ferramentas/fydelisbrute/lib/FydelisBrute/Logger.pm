...
has level => (
    is      => 'ro',
    isa     => class_type('FydelisBrute::Logger') || sub { 1 },
    default => INFO,
);
...
sub _build__fh {
    ...
    open my $fh, '>>', $self->logfile
        or die "Não foi possível abrir logfile '$$self{logfile}': $!";
    $fh->autoflush(1);
    return $fh;
}
...
sub progress {
    ...
    my $cleaned = $message;
    $cleaned =~ s/\r//g;
    # Não poluir o arquivo com progresso
    print "\r\033[K$cleaned";
}
